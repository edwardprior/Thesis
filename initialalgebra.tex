...
\begin{cor} \label{epi} $\eta$ is an epimorphism in $\mathrm{E}G\mathrm{Alg}_S$.
\end{cor}
\begin{proof}
...
\begin{defn} For a given monoid $M$, let $\mathrm{B}M$ represent the one-object category whose morphisms are the elements of $M$, with monoid multiplication as composition. Likewise, for any monoid homomorphism $h: M \to M'$ between abelian groups, denote by $\mathrm{B}h : \mathrm{B}M \to \mathrm{B}M'$ the obvious monoidal functor which acts like $h$ on morphisms. This defines a functor $\mathrm{B}: \mathrm{Mon} \to \mathrm{Cat}$ from the category of monoids onto the category of small categories.
