...
\end{prop}
\begin{proof}
First, note that the above descriptions of $i$ and $d$ are enough to specify them uniquely, because $\mathbb{G}_n$ is the free $\mathrm{E}G$-algebra on $n$ objects and so maps $\mathbb{G}_n \to \mathbb{G}_{2n}$ are canonically isomorphic to functors $\{z_1, ..., z_n\} \to \mathbb{G}_{2n}$. Moreover, we can be sure that the map $q$ exists because $\mathrm{E}G\mathrm{Alg}_S$ is a locally finitely presentable category, and thus has all finite colimits.

Next, let $\psi: \mathbb{G}_n \to X$ be an arbitrary object of $(\mathbb{G}_n \downarrow \mathrm{inv})$. The map $\psi^*: \mathbb{G}_n \to X$ which takes values
...
and hence $(\psi + \psi^*) \circ d$  is just the zero map from $ \mathbb{G}_n$ to $X$. However, we've already defined $q: \mathbb{G}_{2n} \to \mathbb{G}_{2n}/\mathbb{G}_n$ to be the cokernel $d$, the universal map such that $q \circ d$ is zero. Therefore, there must exist a unique map $u: \mathbb{G}_{2n}/\mathbb{G}_n \to X$ making the righthand triangle in following diagram commute:
\begin{eq*} \begin{tikzcd}
...
Now, it should be noted that we have not given a method for actually taking quotients of $\mathrm{E}G$-algebras, and so \cref{quotient} unfortunately does not provide an explicit description of $L\mathbb{G}_n$. Nevertheless, this new perspective still reveals many important facts that are far less obvious from the initial object point of view. Firstly, we have the objects of $L\mathbb{G}_n$:
...
\end{prop}
\begin{proof}
Consider the underlying monoid homomorphism of $d: \mathbb{G}_n \to \mathbb{G}_{2n}$ on objects, $d_{\mathrm{ob}}: \mathbb{N}^{\ast n} \to \mathbb{N}^{\ast 2n}$. This map is an inclusion, and so we can take the quotient of $\mathbb{N}^{\ast 2n}$ with respect to $d_{\mathrm{ob}}$:
\begin{eq*} \mathbb{N}^{\ast 2n}/\mathbb{N}^{\ast n} \, = \, \mathbb{N}^{\ast 2n}/\langle z_i \otimes z_{i+n} \rangle \, = \, \mathbb{Z}^{\ast n} \end{eq*}
But $\mathrm{E}G$-algebra maps out of $\mathbb{G}_n$ are determined entirely by their value on objects, so it follows that the quotient map of the inclusion $d_{\mathrm{ob}}$ is just the underlying monoid homomorphism of the quotient map $q$ of the $d$. Therefore, $\mathrm{Ob}(L\mathbb{G}_n) = \mathbb{Z}^{\ast n}$. 

Moreover, the free monoid generated by objects of the form $z_i \otimes z_{i+n}$, $1 \leq i \leq n$, does not contain any of the elements of the free monoid on the original generators $z_i$, $1 \leq i \leq n$. Thus when we take the quotient of $\mathbb{N}^{\ast 2n}$ by $\langle z_i \otimes z_{i+n} \rangle$ , the submonoid $\langle z_i: 1 \leq i \leq n \rangle = \mathbb{N}^{\ast n}$ will be left unaffected. Therefore, for any $z_i$ with $1 \leq i \leq n$,
\begin{eq*} \eta_{\mathrm{ob}}(z_i) \, = (q \circ i)_{\mathrm{ob}}(z_i) \, = \, q_{\mathrm{ob}} i_{\mathrm{ob}}(z_i) \, = \, q_{\mathrm{ob}}(z_i) \, = \, z_i \end{eq*}
and so the underlying monoid homomorphism of $\eta$ is just the evident inclusion.
\end{proof}

This result makes concrete the sense in which the functor $L$ represents `freely adding inverses' to objects. Specifically, to get the object monoid of $L\mathbb{G}_n$ we have to take the \emph{group completion} of the objects $\mathbb{N}^{\ast n}$ of $\mathbb{G}_n$. 

The second immediate consequence of \cref{quotient} concerns the morphisms $L\mathbb{G}_n$:

\begin{prop}\label{allmapsaction} Every morphism in $L\mathbb{G}_n$ can be expressed as $\alpha_{L\mathbb{G}_n}(g; \mathrm{id}_{w_1}, ..., \mathrm{id}_{w_m})$, for some $g \in G(m)$ and $w_i \in \mathbb{Z}^{*n}$.
\end{prop}
\begin{proof} Let $f$ be an arbitrary morphism in $L\mathbb{G}_n$. Since $q: \mathbb{G}_{2n} \to L\mathbb{G}_n$ is a cokernel, we know it must be a full functor, and so there exists at least one morphism $f'$ in $\mathbb{G}_{2n}$ such that $q(f') = f$. From the definition of $\mathbb{G}_{2n}$ we know that this $f'$ can be expressed uniquely as $\alpha(g; \mathrm{id}_{w'_1}, ..., \mathrm{id}_{w'_m})$ for some $g \in G(m)$ and $w'_1, ..., w'_m \in \mathrm{Ob}(\mathbb{G}_{2n})$, and hence because $q$ is a map of $\mathrm{E}G$-algebras, we will have
\begin{eq*} f \, = \, q(f') \, = \, q \big( \, \alpha_{\mathbb{G}_{2n}}(g; \mathrm{id}_{w'_1}, ..., \mathrm{id}_{w'_m}) \, \big)  \, = \, \alpha_{L\mathbb{G}_n}(g; \mathrm{id}_{q(w'_1)}, ..., \mathrm{id}_{q(w'_m)}) \end{eq*}
Therefore, there is at least one collection of $w_i = q(w'_i)$ for which the proposition holds. 
\end{proof}
...
\begin{prop}\label{concomp} The connected components of $\mathbb{G}_n$ are $\mathbb{N}^n$, with the assignment $[ \,\, ] : \mathbb{N}^{*n} \to \mathbb{N}^n$ of objects to their component being the quotient map of abelianisation. Similarly, the connected components of $L\mathbb{G}_n$ are $\mathbb{Z}^n$, with its assignment of objects to components also given by abelianisation, and with the restriction of $\eta$ to components $\eta_\pi : \mathbb{N}^n \to \mathbb{Z}^n$ being the obvious inclusion. 
\end{prop}
\begin{proof}All morphisms in $\mathbb{G}_n$ are of the form $\alpha(g; \mathrm{id}_{w_1}, ..., \mathrm{id}_{w_m})$ for some $g \in G(m)$ and $w_i \in \mathbb{N}^{*n}$. Since these have source $w_1 \otimes ... \otimes w_m$ and target $w_{\pi(g)^{-1}(1)} \otimes ... \otimes w_{\pi(g)^{-1}(m)}$, we see that it is only possible for two objects to be in the same connected component if they can expanded as a tensor product in ways that are permutations of one another. Moreover, for any two objects where this is true --- say source $w = w_1 \otimes ... \otimes w_m$ and target $w' = w_{\sigma^{-1}(1)} \otimes ... \otimes w_{\sigma^{-1}(m)}$ --- we can always find a map $\alpha(g; \mathrm{id}_{w_1}, ..., \mathrm{id}_{w_m})$ between them, because $\pi: G(m) \to S_m$ is surjective and so there exists $g$ with $\pi(g) = \sigma$. Thus two objects of $\mathbb{G}_n$ share a connected component if and only if they have expansions that differ by a permutation, and therefore the canonical map $[ \,\, ] : \mathrm{Ob}(\mathbb{G}_n) \to \pi_0(\mathbb{G}_n)$ sending each object to its connected component is just the map which forgets about these permutations, making the free product on $\mathbb{N}^{*n}$ commutative. That is, it is the quotient map for the abelianisation $\mathrm{ab} : \mathbb{N}^{*n} \to (\mathbb{N}^{*n})^{\mathrm{ab}}$, and so $\pi_0(\mathbb{G}_n) = \mathbb{N}^n$. 

By \cref{allmapsaction}, all of the morphisms in $L\mathbb{G}_n$ can be written as $\alpha(g; \mathrm{id}_{w_1}, ..., \mathrm{id}_{w_m})$ as well, and so the same proof works there too, giving $\pi_0(Z) = \mathbb{Z}^n$ and $[ \,\, ]_Z = \mathrm{ab} : \mathbb{Z}^{*n} \to \mathbb{Z}^n$. Also, by \cref{Zobj}, $\eta$ acts as an inclusion on objects, so we have the following commutative square:
\begin{eq*} \begin{tikzcd}
\mathbb{N}^{*n} \ar[r, "\mathrm{ab}"] \ar[d, "i"] & \mathbb{N}^{n} \ar[d, "\eta_\pi"] \\
\mathbb{Z}^{*n} \ar[r, "\mathrm{ab}"] & \mathbb{Z}^{n}
\end{tikzcd} \end{eq*}
Thus $\eta_\pi(x^\mathrm{ab}) = x^\mathrm{ab}$, and because $\mathrm{ab}$ is surjective this means that $\eta_\pi$ is an inclusion.
\end{proof}
