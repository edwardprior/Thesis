...
\mathrm{ob}(X) \ar[rr, "F_{\mathrm{ob}}"] \ar[d, "q^X"'] & & \mathrm{ob}(Y) \ar[d, "q^Y"] \\
\pi_0(X) \ar[rr, "F_\pi \, = \, (F_{\mathrm{ob}})^{\mathrm{ab}}"] & & \pi_0(Y)
\end{tikzcd} \end{eq*}
...
\begin{eq*} C(\mathbb{G}_n) \to C_{\mathrm{m}}(\mathbb{G}_n) \end{eq*}
...
\begin{defi} Let $\boxtimes$ be the weak monoidal product defined on $\mathrm{sk}(X)$ by
\begin{eq*} \xymatrix{
...
\begin{eq*} \begin{array}{rcccl}
		F_{\mathrm{po}} & : &  \mathrm{po}(X) & \to & \mathrm{po}(Y) \\
		& : & x & \mapsto & F(x) \\
...
\begin{eq*} \begin{array}{rllrrlll}
		...
		& = & F_{\mathrm{po}}(x) & & & = & F(x) \to F(x') \\
		& = & F_{\mathrm{po}} P^X(x) & & & = & F_{\mathrm{po}}(x \to x') \\
		& & & & & = & F_{\mathrm{po}} P^X(f: x \to x') \\
		& & & & & & \\
		\left[ \, F_{\mathrm{po}}(x) \, \right] & = & [ \, F(x) \, ] & & [ \, F_{\mathrm{po}}(x \to x') \, ] & = & [ \, F(x) \to F(x') \, ] \\
...
\begin{eq*} \begin{tikzcd}
\mathrm{Ob}(X) \ar[rr, "F_{\mathrm{ob}}"] \ar[dd, "q"', bend right = 90] \ar[d, "R^X_{\mathrm{ob}}"] & & \mathrm{Ob}(Y) \ar[dd, "q",  bend left = 90] \ar[d, "R^Y_{\mathrm{ob}}"'] \\
\mathrm{Ob}\big( \, \mathrm{sk}(X) \, \big) \ar[rr, "(F_s)_{\mathrm{ob}}"] \ar[d, "\lbrack \, \_ \, \rbrack_{\mathrm{ob}}", "\rotatebox{90}{\(\sim\)}"'] & & \mathrm{Ob}\big( \, \mathrm{sk}(Y) \, \big) \ar[d, "\lbrack \, \_ \, \rbrack_{\mathrm{ob}}"', "\rotatebox{90}{\(\sim\)}"] \\
\pi_0(X) \ar[rr, "F_\pi"] & & \pi_0(Y)
\end{tikzcd} \end{eq*}
...
\begin{eq*} \begin{tikzcd}
X(s, s) \ar[rr, "F"] \ar[d, "(\rho^X_s)^{-1} \, \circ \, \_ \, \circ \, \rho^X_s"', "\rotatebox{90}{\(\sim\)}"] & & Y\big( F(s), F(s) \big) \ar[d, "(\rho^Y_{F(s)})^{-1} \, \circ \, \_ \, \circ \, \rho^Y_{F(s)}",  "\rotatebox{90}{\(\sim\)}"'] \\
X(s, s) \ar[rr, "F_{\mathrm{sk}}"] \ar[d] & & Y\big( RF(s), RF(s) \big) \ar[d] \\
1 \ar[rr, equal] & & 1
\end{tikzcd} \end{eq*}
...
\begin{eq*} F_{\mathrm{sk}}(f) \, = \, (\rho^Y_{F(s)})^{-1} \circ F(\rho^X_s) \circ F(f) \circ F(\rho^X_s)^{-1} \circ \rho^Y_{F(s)}  \end{eq*}
...
\begin{eq*} \begin{tikzcd}
X(x, x') \ar[rr, "F"] \ar[d, "(\rho^X_{x'})^{-1} \, \circ \, \_ \, \circ \, \rho^X_{x}"', "\rotatebox{90}{\(\sim\)}"] & & Y\big( F(x), F(x') \big) \ar[d, "(\rho^Y_{F(x')})^{-1} \, \circ \, \_ \, \circ \, \rho^Y_{F(x)}",  "\rotatebox{90}{\(\sim\)}"'] \\
X\big( R(x), R(x) \big) \ar[rr, "F_{\mathrm{sk}}"] & & Y\big( RF(x), RF(x) \big)
\end{tikzcd} \end{eq*}
...
\begin{eq*}\begin{array}{rclrrcl}
		R^Y(\mu^F) \circ \mu^{R^Y} & = & F_{\mathrm{sk}}(\mu^{R^X}) \circ \mu^{F_{\mathrm{sk}}}, & \quad & R^Y(\eta^F) \circ \eta^{R^Y} & = & F_{\mathrm{sk}}(\eta^{R^X}) \circ \eta^{F_{\mathrm{sk}}} \\
		\lbrack \mu^{F_{\mathrm{sk}}} \rbrack \circ \mu^{[ \, \_ \, ]} & = & F_\pi(\mu^{[ \, \_ \, ]}) \circ \mu^{F_\pi}, & \quad & [\eta^{F_{\mathrm{sk}}}] \circ \eta^{[ \, \_ \, ]} & = & F_\pi(\eta^{[ \, \_ \, ]}) \circ \eta^{F_\pi}
		\end{array}
\end{eq*}
...
\begin{eq*} \begin{array}{rclrrcl}
		\mu^F_{x, x'} & = & \mathrm{id}_{F(x \otimes x')} & & \eta^F & = & \mathrm{id}_I \\
		\mu^{R^X}_{x, x'} & = & R^X( \rho^X_x \otimes \rho^X_{x'} ) & \quad & \eta^{R^X} & = & \mathrm{id}_{R^X(I)} \\
		\mu^{R^Y}_{y, y'} & = & R^Y( \rho^Y_y \otimes \rho^Y_{y'} ) & \quad & \eta^{R^Y} & = & \mathrm{id}_{R^Y(I)}
		\end{array}
\end{eq*}
Thus,
\begin{eq*}\begin{array}{rcl}
		R^Y(\mathrm{id}_I) \circ \mathrm{id}_{R^Y(I)} & = & F_{\mathrm{sk}}(\mathrm{id}_{R^X(I)}) \circ \eta^{F_{\mathrm{sk}}} \\
		\implies \quad \eta^{F_{\mathrm{sk}}} & = & \mathrm{id}_{R^Y(I)} \\
		& & \\
		R^Y(\mathrm{id}_{F(x \otimes x')}) \circ R^Y( \rho^Y_{F(x)} \otimes \rho^Y_{F(x')} ) & = & F_{\mathrm{sk}} R^X( \rho^X_x \otimes \rho^X_{x'} ) \circ \mu^{F_{\mathrm{sk}}}_{R^X(x), R^X(x')} \\
		& = & R^Y F( \rho^X_x \otimes \rho^X_{x'} ) \circ \mu^{F_{\mathrm{sk}}}_{R^X(x), R^X(x')} \\
		\implies \quad \mu^{F_{\mathrm{sk}}}_{R^X(x), R^X(x')} & = & R^Y\big( \, F( \rho^X_x \otimes \rho^X_{x'} )^{-1} \circ (\rho^Y_{F(x)} \otimes \rho^Y_{F(x')} ) \, \big) \\
...
\begin{eq*} \begin{array}{rll}
		F_{\mathrm{sk}}(f) & = & (\rho^Y_{F(r)})^{-1} \circ F(\rho^X_r) \circ F(f) \circ F(\rho^X_r)^{-1} \circ \rho^Y_{F(r)}  \\
...
\begin{eq*} \begin{tikzcd}
X(x, x') \ar[rrr, "F"] \ar[d, "(\rho^X_{x'})^{-1} \, \circ \, \_ \, \circ \, \rho^X_{x}"', "\rotatebox{90}{\(\sim\)}"] & & & Y\big( F(x), F(x') \big) \ar[d, "c_{[r]}^{-1} \circ F(\rho^X_r)^{-1} \, \circ \, \_ \, \circ \, F(\rho^X_x) \circ c_{[x]}",  "\rotatebox{90}{\(\sim\)}"'] \\
X\big( R(x), R(x) \big) \ar[rrr, "c_{[r]}^{-1} \circ F(\, \_ \,) \circ c_{[r]}"] & & & Y\big( RF(x), RF(x) \big)
\end{tikzcd} \end{eq*}
