...
\begin{eq*} \mathbb{G}_n \quad \cong \quad \coprod_{[w] \in \mathbb{N}^n} \mathrm{B}\mathbb{G}_n(w, w) \times \mathrm{E}[w]. \end{eq*}
...
\begin{eq*} X \quad \cong \quad \pi_0(X) \times \mathrm{B}X(I,I) \times \mathrm{E}[I]. \end{eq*}
\end{prop}
\begin{proof}
...
\begin{eq*} H \quad \cong \quad \coprod_{[h] \in \pi_0(H)} \mathrm{B}H(h,h) \times \mathrm{E}[h]. \end{eq*}
...
\begin{eq*} \begin{array}{lrll}
		\_ \otimes x^* : & \mathrm{E}[x] & \to & \mathrm{E}[0] \\
		& x' & \mapsto & x' \otimes x^* \\
...
\begin{eq*} X \quad \cong \quad \coprod_{[x] \in \pi_0(X)} \mathrm{B}X(I, I) \times \mathrm{E}[I]. \end{eq*}
...
\begin{eq*} \psi_\pi : \mathbb{N}^n \to \pi_0(X) \end{eq*}
\item a family of group homomorphisms onto a fixed group,
\begin{eq*} \psi^w : \mathbb{G}_n(w,w) \to X(I,I), \end{eq*}
indexed by the representing objects and satisfying the relation
\begin{eq*} \psi^w(f) = \psi^{w \otimes v}(f \otimes id_v), \quad \quad \forall \, f \in \mathbb{G}_n(w, w) \end{eq*}
...
\begin{eq*}\begin{array}{rrll}
		\psi_\pi : & \pi_0(\mathbb{G}_n) & \to & \pi_0(X) \\
		\mathrm{B}\psi^w : & \mathrm{B}\mathbb{G}_n(w,w) & \to & \mathrm{B}X(I,I) \\
		\mathrm{E}\psi_w : & \mathrm{E}[w] & \to & \mathrm{E}[I]
		\end{array}.
\end{eq*}
...
\begin{eq*} \xymatrix{
\mathbb{G}_n(w,w) \ar[dr]_{\psi^w} \ar[rr]^-{\_ \otimes id_v} & & \mathbb{G}_n(w \otimes v , w \otimes v) \ar[dl]^{\psi^{w \otimes v}} \\
& X(I,I) & }
\end{eq*}
...
\begin{eq*} \xymatrix{
& \mathrm{colim}(D^\bullet) \ar[dl]_{q} \ar[dr]^{v^\bullet} & \\
\mathrm{colim}(D^\bullet)^{\mathrm{ab}} \ar[rr]^{(v^\bullet)^{\mathrm{ab}}} & & X(I, I) }.
\end{eq*}
...
\begin{eq*} \xymatrix{
\big( \, [w], \, w \, \big) \ar[dr]_{\psi_w} \ar[rr]^-{\_ \otimes v} & & \big( \, [w \otimes v], \, w \otimes v \, \big) \ar[dl]^{\psi_{w \otimes v}} \\
& \big( \, [I]_X, \, I \, \big) & }.
\end{eq*}
...
\begin{eq*} \xymatrix{
& \mathrm{colim}(D_\bullet) \ar[dl]_{\iota} \ar[dr]^{v_\bullet} & \\
\mathrm{F}\big( \, \mathrm{colim}(D_\bullet) \setminus \ast \, \big) \ar[rr]^{\mathrm{F}(v_\bullet)} & & \big( \, [I]_X, \, I \, \big) }.
\end{eq*}
