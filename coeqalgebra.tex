\chapter{Free invertible algebras as coequalizers}
\label{coeqalgebra}
...
\section{$L\mathbb{G}_n$ as a coequalizer in $\mathrm{MonCat}$}
...
\begin{eq*} \mathrm{Mor}(L\mathbb{G}_n)^{\mathrm{ab}} \quad = \quad \bigquotient{(G \times_{\mathbb{N}} \mathbb{N}^{\ast 2n})^{\mathrm{gp, ab}}}{\Delta(G,n)} \end{eq*}
\end{prop}
\begin{proof}
